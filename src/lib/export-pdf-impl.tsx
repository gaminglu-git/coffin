/**
 * PDF export implementation - loaded dynamically to reduce initial bundle.
 */
  ...
  return (
    <Document>
      <Page size="A4" style={styles.page}>
        ...
        {subject && <Text style={styles.heading}>{subject}</Text>}
        <Text style={{ ...styles.text, marginBottom: 20 }}>{new Date().toLocaleDateString("de-DE")}</Text>
        {paragraphs.map((p, i) => (
          <Text key={i} style={styles.text}>{p.replace(/\n/g, " ")}</Text>
        ))}
      </Page>
    </Document>
  );
        ...
        {checklists.map((list, i) => (
          <View key={i} style={{ marginBottom: 20 }}>
            ...
            {list.items.map((item, j) => (
              <Text key={j} style={styles.text}>{item.completed ? "☑" : "☐"} {item.text}</Text>
            ))}
          </View>
        ))}
      </Page>
...
export async function exportLetterAsPDFImpl(
  subject: string,
  content: string,
  filename?: string
): Promise<void> {
...
export async function exportChecklistAsPDFImpl(
