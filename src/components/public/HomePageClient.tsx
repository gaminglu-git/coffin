...
import { WebsitePreview } from "@/components/admin/WebsiteEditor";
...
export function HomePageClient({ companySettings, pageContent }: HomePageClientProps) {
  ...
  // Hash-Links öffnen Modals (z.B. von Puck Hero/Leistungen-Links)
  useEffect(() => {
    const checkHash = () => {
      if (typeof window === "undefined") return;
      const hash = window.location.hash;
      if (hash === "#vorsorge") setIsConfiguratorOpen(true);
      else if (hash === "#trauerfall") setIsTrauerfallOpen(true);
      else if (hash === "#beratung") setIsBeratungOpen(true);
    };
    checkHash();
    ...
    <div className="pt-20 bg-stone-50 min-h-screen">
      <section id="hero" className="relative min-h-[80vh] bg-stone-50 flex items-center justify-center py-24 sm:py-32 lg:pb-40 text-center px-4 overflow-hidden">
        ...
        <div className="relative z-10 max-w-3xl mx-auto space-y-8">
          ...
          <div className="flex flex-col sm:flex-row justify-center gap-4">
            <button onClick={() => scrollTo("kontakt")} className="px-8 py-4 bg-rose-200 text-stone-800 rounded-full font-medium hover:bg-rose-300 transition-colors shadow-sm hover:shadow-md flex items-center justify-center gap-2">
              <Phone size={20} /> Lassen Sie uns reden
            </button>
            <button onClick={() => setIsConfiguratorOpen(true)} className="px-8 py-4 bg-white text-emerald-800 rounded-full font-medium hover:bg-stone-100 border border-stone-200 transition flex items-center justify-center gap-2 shadow-sm">
      ...
      <section id="leistungen" className="relative py-24 bg-stone-100 px-4 overflow-hidden">
        <div className="absolute top-8 right-16 opacity-20 pointer-events-none hidden md:block">
          <Image src="/assets/eucalyptus.svg" alt="" width={56} height={56} className="w-14 h-14" />
        </div>
        <div className="max-w-7xl mx-auto flex flex-col md:flex-row items-center gap-16 relative z-10">
          <div className="md:w-1/2">
            <h3 className="text-3xl sm:text-4xl font-serif text-emerald-800 mb-6 leading-tight">
              Selbstbestimmt bis zum Schluss. <br />Unsere Vorsorge.
            </h3>
          ...
          <div className="md:w-1/2 w-full bg-white p-8 sm:p-12 rounded-4xl shadow-xl border border-stone-100 text-center" id="vorsorge">
            ...
            <button onClick={() => setIsConfiguratorOpen(true)} className="bg-emerald-700 text-white px-8 py-4 rounded-full text-lg font-medium hover:bg-emerald-800 transition w-full shadow-md">
              Jetzt Wünsche konfigurieren
            </button>
            <p className="mt-6 text-sm text-stone-500">
              Trauerfall?{" "}
              <button type="button" onClick={() => setIsTrauerfallOpen(true)} className="text-emerald-700 hover:underline font-medium">Sofort anfragen</button>
              {" · "}
              Beratung gewünscht?{" "}
              <button type="button" onClick={() => setIsBeratungOpen(true)} className="text-emerald-700 hover:underline font-medium">Anfragen</button>
            </p>
          </div>
        </div>
      </section>
      ...
      <footer id="kontakt" className="relative bg-stone-200/80 py-20 px-4 overflow-hidden">
        ...
        <div className="absolute bottom-8 left-12 opacity-25 pointer-events-none">
          <Image src="/assets/lavender.svg" alt="" width={80} height={80} className="w-20 h-20" />
        </div>
        <div className="absolute bottom-12 right-1/4 opacity-20 pointer-events-none hidden md:block">
          <Image src="/assets/flower.svg" alt="" width={56} height={56} className="w-14 h-14" />
        </div>
        <div className="absolute top-1/2 left-8 opacity-20 pointer-events-none hidden lg:block">
        ...
        <div className="relative z-10 max-w-7xl mx-auto grid lg:grid-cols-2 gap-16">
          <div>
            ...
            <div className="space-y-6">
              ...
              <div className="flex items-center gap-4 text-stone-700 p-4">
                ...
                <a href="mailto:info@minten-walter.de" className="text-lg font-medium hover:text-emerald-700 transition">info@minten-walter.de</a>
              </div>
            </div>
          </div>
          <div className="bg-white/70 backdrop-blur-sm rounded-4xl p-10 text-center shadow-lg border border-stone-200/80 flex flex-col justify-center">
            ...
            <a href="tel:02286205815" className="inline-block bg-rose-200 text-stone-800 px-8 py-3.5 rounded-full font-medium hover:bg-rose-300 transition mx-auto shadow-md">Jetzt anrufen</a>
          </div>
        </div>
      </footer>
    </div>
  ...
  return (
    <>
      <Navbar displayName={companySettings.displayName} tagline={companySettings.tagline} />
      {pageContent && pageContent.content?.length > 0 ? (
        <WebsitePreview data={pageContent} />
      ) : (
        renderFallback()
      )}
